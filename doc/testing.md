...
This guide is lifted from pretalx's contributor docs. pxtx is a much smaller
project, but the same philosophy applies: tests are documentation, tests are
fast, and tests verify behaviour. The wording below is preserved verbatim from
the source; sections that talk about pretalx-specific infrastructure (event
fixtures, scopes middleware, Celery, etc.) are adapted for pxtx.
...
etc. For the REST API, integration tests cover each endpoint's happy path plus
at least one auth-failure case.

**End-to-end tests:** Multi-step user flows that cross view boundaries, still
using pytest and the Django test client (no browser automation). For pxtx,
these encode the critical paths: creating an issue via API, commenting on it,
reassigning through status transitions, referencing it from another issue, and
verifying the ActivityLog captures every step.
...
- Database state changed as expected
  (`assert Issue.objects.filter(status='completed').count() == 1`, plus
  checking that the completed issue is the one that **should** be completed)
- The correct template was used
  (`assert 'core/issue_detail.html' in [t.name for t in response.templates]`)
...
- Response body contains expected content for API views
  (`assert response.json()['results'][0]['slug'] == 'PX-47'`)
...
```python
# Bad – passes even if other issues snuck in
assert issue in milestone.issues.all()

# Good – verifies the exact set
assert list(milestone.issues.all()) == [issue]
assert set(milestone.issues.all()) == {issue1, issue2}
```
...
Tests live in `src/tests/`, mirroring the code structure and named to match the
file they test. For example, the views in `src/pxtx/core/views/issue.py` are
tested in `src/tests/views/test_issue.py`.
...
`src/tests/views/unit/test_issue.py` and
`src/tests/views/integration/test_issue.py` rather than
`test_issue.py` and `test_issue_integration.py`. This keeps filenames clean
and makes it easy to run an entire layer at once
(`just test src/tests/views/integration/`).

Test functions are named `test_<thing>_<condition_or_behaviour>`:

- `test_issue_slug_follows_px_format`
- `test_api_create_issue_without_token_returns_401`
- `test_kanban_reorder_emits_single_activity_entry`
...
[coverage.py](https://coverage.readthedocs.io/) for coverage tracking. 100%
coverage is required for CI to pass.
...
For API tests, DRF's `APIClient` is preferred over the plain Django client; a
`token_client` fixture in `conftest.py` wraps it with
`Authorization: Token <plaintext>` set to a freshly-minted token.

**pytest fixtures** for composing test setups. Complex arrangements (e.g. a
milestone with several issues across statuses, plus cross-references) live as
fixtures in `conftest.py` at the appropriate level. We do **not** use fixtures
to wrap a single factory call — those go directly in the test.
...
- `just test -k <pattern>`: run only tests matching a name pattern.
...
- `just test --no-header -rN`: minimal output, useful when running the full
  suite.
- `just test tests/views/`: run all tests for one area.
